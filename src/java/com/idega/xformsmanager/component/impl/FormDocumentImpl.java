...
/**
 * TODO: put isLazyLoaded(true) everywhere the loading of the document is needed for further
...
public class FormDocumentImpl extends FormComponentContainerImpl implements
	...
	public XFormsManagerDocument getXFormsManager() {
		DMContext context = getContext();
		XFormsManagerFactory factory = context.getXformsManagerFactory();
		XFormsManagerDocument doc = factory.getXformsManagerDocument();
		return doc;
	}
		...
		if (!localizedComponentsDocuments.containsKey(locale)) {
			...
			try {
				...
				Logger.getLogger(getClass().getName()).log(Level.SEVERE,
				    "Exception while generating components html document", e);
	...
	/**
	 * checks if document is loaded. If loadIfNotLoaded is true, then it loads the document if it
	 * isn't already
	 *
