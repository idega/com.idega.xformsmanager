...
public class FormComponentButtonAreaImpl extends FormComponentContainerImpl
	...
	public Button getButton(ConstButtonType buttonType) {
		...
		return !getButtonsTypeIdMapping().containsKey(buttonType) ? null
				: (Button) getContainedComponent(getButtonsTypeIdMapping().get(
						buttonType));
	}
	...
	public Button addButton(ConstButtonType buttonType, String nextSiblingId) {
		...
		if (getButtonsTypeIdMapping().containsKey(buttonType))
			throw new IllegalArgumentException("Button by type provided: "
					+ buttonType + " already exists in the button area");
	...
	public void remove() {
		super.remove();
		getParent().setButtonAreaComponentId(null);
	}
