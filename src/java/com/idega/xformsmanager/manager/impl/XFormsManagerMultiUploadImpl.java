...
public class XFormsManagerMultiUploadImpl extends XFormsManagerImpl implements XFormsManagerMultiUpload {
	...
	private final XPathUtil insertElementXPath = new XPathUtil(
	        ".//xf:insert[@at]");
	private final XPathUtil deleteElementXPath = new XPathUtil(
	        ".//xf:delete[@at]");

	private void updateNodesetReferencingAttributes(FormComponent component) {

		String nodesetName = component.getComponentDataBean()
		        .getComponentBind().getBind().getNodeset().getNodesetElement()
		        .getNodeName();

		Element insertElement = insertElementXPath.getNode(component
		        .getComponentDataBean().getElement());
		Element deleteElement = deleteElementXPath.getNode(component
		        .getComponentDataBean().getElement());

		String insertNodeset = insertElement
	...
	protected void updateAddButtonLabel(FormComponent component) {
		...
		FormManagerUtil.putLocalizedText(FormManagerUtil.ref_s_att, null, null,
		    addButtonlabel, component.getFormDocument().getXformsDocument(),
		    localizedText);
	...
	protected void updateRemoveButtonLabel(FormComponent component) {
		...
		FormManagerUtil.putLocalizedText(FormManagerUtil.ref_s_att, null, null,
		    removeButtonlabel, component.getFormDocument().getXformsDocument(),
		    localizedText);
	...
	protected void updateUploadingFileDescription(FormComponent component) {
		...
		FormManagerUtil.putLocalizedText(FormManagerUtil.ref_s_att, null, null,
		    label, component.getFormDocument().getXformsDocument(),
		    localizedText);
	...
	protected void updateUploaderHeaderText(FormComponent component) {
		...
		FormManagerUtil.putLocalizedText(FormManagerUtil.ref_s_att, null, null,
		    headerElem, component.getFormDocument().getXformsDocument(),
		    localizedText);
	...
	public LocalizedStringBean getUploaderHeaderText(FormComponent component) {
		...
		return FormManagerUtil.getElementLocalizedStrings(
		    getUploadersHeaderElement(component), component.getFormDocument()
		            .getXformsDocument());
	}
	...
	private Element getUploadersHeaderElement(FormComponent component) {
		...
		if (headerTextElem == null)
			headerTextElem = new XPathUtil(".//" + FormManagerUtil.output_tag + "[@model='data_model']").getNode(xformsComponentDataBean.getElement());
	...
	protected void updateErrorMsg(FormComponent component, ErrorStringBean errString) {
		...
		Element output = (Element) uploadErrorMsgElementXpath.getNode(xformsComponentDataBean.getElement());
		if (output == null)
			output = new XPathUtil(".//" + FormManagerUtil.output_tag + "[@value]").getNode(xformsComponentDataBean.getElement());;
	...
	protected ComponentBind createComponentBind(FormComponent component,
	        Bind bind) {
		return new ComponentBindMultiUploadImpl(component, bind);
	}
}