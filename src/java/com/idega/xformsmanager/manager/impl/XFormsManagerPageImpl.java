...
@Service
@Scope("singleton")
public class XFormsManagerPageImpl extends XFormsManagerContainerImpl implements
		XFormsManagerPage {
	...
	public void addComponentToDocument(FormComponent component) {
		...
		// Element case_element =
		// componentElement.getOwnerDocument().createElementNS(FormManagerUtil.idega_namespace,
		// FormManagerUtil.idegans_case_tag);
		//		
		//		
		// String name =
		// componentElement.getAttribute(FormManagerUtil.name_att);
		// if(name != null && name.length() != 0) {
		//			
		// componentElement.removeAttribute(FormManagerUtil.name_att);
		// case_element.setAttribute(FormManagerUtil.name_att, name);
		// }
		// componentElement.getParentNode().replaceChild(case_element,
		// componentElement);
	...
	protected void checkForSpecialTypes(FormComponent component) {
		...
		String pageType = xformsComponentDataBean.getElement().getAttribute(
				FormManagerUtil.type_att);
		if (pageType != null
	...
	/*
	 * @Override public void removeComponentFromXFormsDocument(FormComponent
	 * component) {
	 * 
	 ...
	 * element_to_remove
	 * .getParentNode().getParentNode().removeChild(element_to_remove
	 * .getParentNode()); }
	 */

	private void removeSectionVisualization(FormComponent component) {

		Element section = FormManagerUtil.getElementById(component
	...
	public void moveComponent(FormComponent component, String nextSiblingId) {
		super.moveComponent(component, nextSiblingId);
	}

	/*
	 * @Override public void moveComponent(FormComponent component, String
	 * nextSiblingId) {
	 * 
	 * ComponentDataBean xformsComponentDataBean =
	 * component.getXformsComponentDataBean();
	 ...
	 * 
	 * Element componentElement = xformsComponentDataBean.getElement(); Element
	 * element_to_insert_before = null;
	 * 
	 * if(nextSiblingId != null) {
	 * 
	 * element_to_insert_before =
	 * FormManagerUtil.getElementByIdFromDocument(xform,
	 * FormManagerUtil.body_tag, nextSiblingId); } else {
	 * 
	 * Element components_container = (Element)componentElement.getParentNode();
	 * element_to_insert_before =
	 * DOMUtil.getLastChildElement(components_container); }
	 * 
	 * Element componentElement =
	 * (Element)((Element)componentElement.getParentNode
	 * ()).insertBefore(componentElement, element_to_insert_before);
	 * 
	 * xformsComponentDataBean.setElement(
	 * (Element)((Element)((Element)componentElement
	 * .getParentNode()).insertBefore(componentElement,
	 * element_to_insert_before))
	...
	// return
	// (Element)component_after_this.getXformsComponentDataBean().getElement().getParentNode();
	// }
	public void pageContextChanged(FormComponent component) {
		...
		if (!formDocument.getProperties().isStepsVisualizationUsed()
				|| FormComponentFactory.page_type_thx.equals(component
						.getType()))
			return;

		Element instance = formDocument
		...
		if (section == null) {
			...
			section = FormManagerUtil.getItemElementById(component
					.getFormDocument().getContext().getCacheManager()
					.getComponentsTemplate(), FormManagerUtil.section_item);
			section = (Element) component.getFormDocument().getXformsDocument()
					.importNode(section, true);
			section.setAttribute(FormManagerUtil.id_att, component.getId()
					+ "_section");
			Element id_el = (Element) section.getElementsByTagName(
					FormManagerUtil.id_att).item(0);
			FormManagerUtil.setElementsTextNodeValue(id_el, component.getId());
		} else
			section.getParentNode().removeChild(section);

		int seq_idx = component.getParent().getContainedComponentsIds()
				.indexOf(component.getId()) + 1;
		...
		Element seq_el = (Element) section.getElementsByTagName("seq_index")
				.item(0);
		FormManagerUtil.setElementsTextNodeValue(seq_el, String
				.valueOf(seq_idx));
		...
		if (next != null)
			inserted = insertSectionIntoContext(component, false, next,
					instance, section);

		if (prev != null && !inserted)
			inserted = insertSectionIntoContext(component, true, prev,
					instance, section);
	...
	private boolean insertSectionIntoContext(FormComponent component,
			boolean prev, FormComponentPage relevant_page, Element instance,
			Element section) {
		...
		if (relevant_page != null) {
			...
			if (!prev)
				relevant_section.getParentNode().insertBefore(section,
						relevant_section);
			else
			...
			Element class_exp = (Element) instance.getElementsByTagName(
					"class_exp").item(0);
