 ...
 * @version $Revision: 1.17 $ Last modified: $Date: 2009/04/28 15:02:05 $ by $Author: arunas $
 */
public class Bind {
	...
	public Nodeset getNodeset() {
		if (nodeset == null) {
			...
			if (StringUtil.isEmpty(nodesetPath)) {
				logger.log(Level.WARNING, "No nodeset attribute for bind element. Bind id=" + getId()  + ", form id=" + getFormDocument().getId());
				return null;
			}
			...
			if (getParentBind() == null) {
				...
				nodeset = getNodesetFactory().locate(model, nodesetPath, getParentBind().getNodeset());
			}

			if (nodeset == null)
				throw new NullPointerException( "Nodeset path (" + nodesetPath  + ") specified in the nodeset attribute, but no nodeset found, form id=" + getFormDocument().getId());
		}
	...
	public boolean isCalculate() {
		return isCalculate == null ? isCalculate = getCalculate().length() != 0 : isCalculate ;
	}
